...
def save_to_txt(data, file_name_txt):
    try: 
...
def download_pubmed():
    ...
    # Toggle button to enable/disable saving as a text file
    save_to_txt_enabled = st.checkbox("Enable Save to txt File")
    ...
    if st.button("Download"):
        ...
        for i, pubmed_id in enumerate(tqdm(record["IdList"], desc="Processing documents")):
            ...
            # Construct XML file's name, typically using PubMed ID
            xml_file_name = os.path.join(folder_path, f"{pubmed_id}.xml")
            txt_file_name = os.path.join(folder_path, f"{pubmed_id}.txt") 

            # Save XML data to a file, using binary mode 'wb' to write byte data
            if save_to_txt_enabled == False:
                with open(xml_file_name, "wb") as xml_file:
                    xml_file.write(xml_data)
            ...
            if save_to_txt_enabled:
                tree = ET.ElementTree(ET.fromstring(xml_data))
                root = tree.getroot()
                title = root.find(".//ArticleTitle").text
                abstract = root.find(".//AbstractText").text if root.find(".//AbstractText") is not None else ""
                save_to_txt([title, abstract], txt_file_name)
    ...
    if len(path_keywords) > 0: 
        ...
        for path_keyword in path_keywords:
            folder_path = f"dataset/{path_keyword}" 
            # check if folder path is not .DS_Store
            if folder_path != "dataset/.DS_Store":
                file_count = len(os.listdir(folder_path))
                number_of_files.append(file_count)
